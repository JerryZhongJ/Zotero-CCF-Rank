...
function fetchRank( title, authorA, year, site) {
...
function fetchRank(title, authorA) {
    return fetch("https://dblp.org/search/publ/api?q=" + encodeURIComponent(title + "  author:" + authorA) + "&format=json")
}

function *getDblpInfos(response) {
    if (response.ok) {
...
function getRank(dblp_url, dblp_number, dblp_venue) {
    dblp_url = ccf.rankDb[dblp_url]
    if(typeof(dblp_url) == "undefined" && resp_flag != false){
        dblp_abbr = resp.hit[0].info.number;
        if(typeof(dblp_abbr) != "undefined" && isNaN(dblp_abbr)){
